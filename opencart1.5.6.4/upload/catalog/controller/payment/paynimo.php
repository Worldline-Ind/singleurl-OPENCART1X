...
class ControllerPaymentPaynimo extends Controller {
	public function index() {
	    ...
	    if($merchant_details[0]['webservice_locator'] == 'Test'){
		    ...
		    //$shoppingCartStr = $merchant_details[0]['merchant_scheme_code'].'_1.00_0.0';
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paynimo.tpl')) {
		    $this->template = $this->config->get('config_template') . '/template/payment/paynimo.tpl';
		} else {
		    $this->template = 'default/template/payment/paynimo.tpl';
		}
	...
	public function getResponse() {
	    ...
	    if($_POST){
			...
			$this->load->model('payment/paynimo');
	        if($status == '300') {
				// $this->model_payment_paynimo->confirm($this->session->data['order_id'], $this->config->get('paynimo_order_status_complete'));
	            echo "<script>window.location = '".$this->url->link('checkout/success')."'</script>";
	        } else if($status == '392') {
				$this->model_payment_paynimo->confirm($this->session->data['order_id'], $this->config->get('paynimo_order_status_cancel'));
				echo "<script>window.location = '".$this->url->link('checkout/failure')."'</script>";
			} else if($status == '397') {
				$this->model_payment_paynimo->confirm($this->session->data['order_id'], $this->config->get('paynimo_order_status_abort'));
				echo "<script>window.location = '".$this->url->link('checkout/failure')."'</script>";
			} else {
				$this->model_payment_paynimo->confirm($this->session->data['order_id'], $this->config->get('paynimo_order_status_failure'));
				echo "<script>window.location = '".$this->url->link('checkout/failure')."'</script>";
			}
	    }
	}
	...
	public function confirm() {
	    ...
	    $this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('paynimo_order_status_confirm'));
	}
}